...
const { cmdNotFound, owner, donate, down, help, admins, adult, maker, animes, outros, animal, Akami, letras, estudo, stic, downa, readme, lang, convh } = require('./lib/help')
...
module.exports = kconfig = async (kill, message) => {
    try {
        ...
        const mess = {
            wait: 'tá ok! Chega de café, só espera aew pq o café tá caro pra caralho!',
            error: {
                St: `Mano(a) c é burro(a)?\nPara usar isso, envie ou marque uma foto com essa mensagem, se for um gif, use o comando ${prefix}gif.`,
                Ki: 'Atá vc quer tirar um ADM?, só pvc saber primeiro tem que remover o ADM deles! Tente a sorte!.',
                Ad: 'Erros! Não pude adicionar, pode ser por limitação de adicionar ou erros meus.',
                Go: 'Opa peraí, vc não é o dono do grupo, apenas o dono de um grupo pode usar esse tipo de comando.',
				Kl: 'Opa! Isso é apenas meu dono, você não pode acessar.',
				Ga: 'Apenas Administradores podem usar, então trate de virar um haha!',
				Gp: 'véi tu tá usando no privado, mas isso é um comando para grupos.',
				Ac: `Somente grupos que permitem conteúdo +18 podem usar comandos assim, se você é o dono e quer isso, use ${prefix}nsfw enable, ou se vc é punheteiro oculto use no PV.`,
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isLeg && !isOwner) {
			try {
				if (chats.match(new RegExp(/(https:\/\/chat.whatsapp.com)/gi))) {
					...
					if (gplka) {
						console.log(color('[BAN]', 'red'), color('Link de grupo detectado, metendo o pé nesse FDP!...', 'red'))
						await kill.removeParticipant(groupId, sender.id)
					} else {
						console.log(color('[ALERTA]', 'yellow'), color('Link de grupo invalido recebido...', 'red'))
					}
				}
			} catch (error) {
				return
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isLeg && !isOwner) {
			try {
				if (isUrl(chats)) {
					...
					console.log(color('[LINK]', 'cyan'), 'Link recebido:', inilkn.hostname)
					isPorn(inilkn.hostname, async (err, status) => {
						if (err) return console.error(err)
						if (status) {
							console.log(color('[NSFW]', 'red'), color('O link contém pornografia dentro, removendo punheteiro...', 'red'))
							await kill.removeParticipant(groupId, sender.id)
						} else {
        ...
        // ANTI FLOOD PRIVADO
        if (isCmd && msgFilter.isFiltered(from) && !isGroupMsg) { return console.log(color('FLOOD AS', 'red'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'cyan'), color(`${command} [${args.length}]`), 'de', color(pushname)) }
		
		// ANTI FLOOD GRUPOS
        if (isCmd && msgFilter.isFiltered(from) && isGroupMsg) { return console.log(color('FLOOD AS', 'red'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'cyan'), color(`${command} [${args.length}]`), 'de', color(pushname), 'em', color(name || formattedTitle)) }
		
		
        // MENSAGEM PV
        if (!isCmd && !isGroupMsg) { return console.log('> MENSAGEM AS', color(moment(t * 5000).format('DD/MM/YY HH:mm:ss'), 'cyan'), 'de', color(pushname)) }
		
		// MENSAGEM GP
        if (!isCmd && isGroupMsg) { return console.log('> MENSAGEM AS', color(moment(t * 5000).format('DD/MM/YY HH:mm:ss'), 'cyan'), 'de', color(pushname), 'em', color(name || formattedTitle)) }
		
		
		// COMANDOS
        if (isCmd && !isGroupMsg) { console.log(color(`> COMANDO "${command} [${args.length}]" AS`), color(moment(t * 5000).format('DD/MM/YY HH:mm:ss'), 'cyan'), 'de', color(pushname)) }
		
		// COMANDOS GP
        if (isCmd && isGroupMsg) { console.log(color(`> COMANDO "${command} [${args.length}]" AS`), color(moment(t * 5000).format('DD/MM/YY HH:mm:ss'), 'cyan'), 'de', color(pushname), 'em', color(name || formattedTitle)) }
        ...
        case 'wasted':
            if (isMedia && type === 'image' || isQuotedImage) {
                ...
                await kill.sendFileFromUrl(from, `https://some-random-api.ml/canvas/wasted?avatar=${wasteup.url}`, 'Wasted.jpg', 'Alguém viu essa porra por aqui?', id)
            } else {
        ...
        case 'gif':
            ...
            if (isMedia && type === 'video' || mimetype === 'image/gif' || isQuotedVideo || isQuotedGif) {
                ...
                await kill.reply(from, 'Burro! Isso somente pode ser usado com videos e gifs.', id)
            }
            break
		...
		case 'simg':
            ...
            if (isMedia && type === 'image' || isQuotedImage) {
				...
				await kill.reply(from, ' ~Burro~, isso somente funciona com imagens.', id)
			}
			break
		...
		case 'upimg':
            ...
            if (isMedia && type === 'image' || isQuotedImage) {
				...
				await kill.reply(from, ' ~Burro~(a), isso somente funciona com imagens.', id)
			}
			break
        ...
        case 'stalktw':
            ...
            const tw = await axios.get(`http://killz.my.id/api/media/stalktwitt?user=${body.slice(9)}`)
        ...
        case 'twitter':
            ...
            const twi = await axios.get(`http://killz.my.id/api/media/twvid?url=${body.slice(4)}`)
        ...
        case 'ig':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
            if (args.length == 0) return kill.reply(from, 'Cade o link né?', id)
            const iga = await axios.get(`https://api.zeks.xyz/api/ig?url=${body.slice(5)}&apikey=apivinz`)
       ...
       case 'translate':
        ...
        case 'dono':
			...
			kill.reply(from, 'Se ele não responder apenas espere, é raro ele sair da internet.', id)
            break
        ...
        case 'nsfw':
			...
			if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
				if (args[0].toLowerCase() == 'enable') {
					...
					kill.reply(from, 'Comandos NSFW ativados para os punehteiros deste grupo!', id)
				} else if (args[0].toLowerCase() == 'disable') {
					nsfw_.splice(chat.id, 1)
					fs.writeFileSync('./lib/config/NSFW.json', JSON.stringify(nsfw_))
					kill.reply(from, 'Comandos NSFW desativamos para os benga moles deste grupo.', id)
				} else {
      ...
      case 'moddroid':
		...
		case 'bcgc':
			...
			if (isMedia && !mek.message.videoMessage || isQuotedImage) {
				...
				buff = await kill.downloadMediaMessage(encmedia)
				for (let _ of anu) {
					kill.sendMessage(_.jid, buff, image, {caption: `*「 BC GRUPO 」*\n\nDo grupo : ${groupName}\nRemetente : wa.me/${(sender.split('@')[0])}\nMensagem : ${body.slice(6)}`})
				}
				reply('')
			} else {
				for (let _ of anu) {
					sendMess(_.jid, `*「 BROADCAST GRUPO 」*\n\nDo grupo : ${groupName}\nRemetente : wa.me/${(sender.split('@')[0])}\nMensagem : ${body.slice(6)}`)
				}
				reply('Enviado!')
			}
			break 
	    ...
	    case 'add':
            ...
            try {
				...
				await kill.sendText(from, '🎉 Bem vindo(a)! 🎉')
            } catch {
...
//		case 'glitch':
			...
			if (args.length >= 2) {
				...
				const glitch = await axios.get(`https://api.zeks.xyz/api/gtext?text1=${fison}&text2=${twoso}&apikey=apivinz`)
				if (fison > 10 || twoso > 10) return kill.reply(from, 'Desculpe, maximo de 10 letras.', id)
				 await kill.sendFileFromUrl(from, `${glitch.data.result}`, '', 'Aqui está!', id) 
			} else {
				await kill.reply(from, `Para usar isso, adicione duas frases, separando elas pelo |.`, id)
			}
			break
...
//		case 'pornhub':
			...
			if (args.length >= 2) {
				...
				const fison = arkp.split('|')[0]
				const twoso = arkp.split('|')[1]
				const phlogo = await axios.get(`https://api.zeks.xyz/api/phlogo?text1=${fison}&text2=${twoso}&apikey=apivinz`)
				if (fison > 10 || twoso > 10) return kill.reply(from, 'Desculpe, maximo de 10 letras.', id)
				 await kill.sendFileFromUrl(from, `${phlogo.data.result}`, '', '', id) 
			} else {
				await kill.reply(from, `Para usar isso, adicione duas frases, separando elas pelo |.`, id)
			}
			break
...
//		case 'pornhub2':
		...
		case 'retro':
				...
				if (args.length >= 3) {
					...
					const fisot = arkt.split('|')[0]
					const twost = arkt.split('|')[1]
					const trest = arkt.split('|')[2]
					const retro = await axios.get(`https://api.zeks.xyz/api/retro?text1=${fisot}&text2=${twost}&text3=${trest}&apikey=apivinz`)
					await kill.sendFileFromUrl(from, `${retro.data.result}`, '', '', id) 
				} else {
					await kill.reply(from, `Para usar isso, adicione três frases, separando elas pelo |.`, id)
				}
				break
		...
		case 'water':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
				...
				if (args.length >= 2) {
					...
					const fison = arkp.split('|')[0]
					const twoso = arkp.split('|')[1]
					const water = await axios.get(`https://api.zeks.xyz/api/watercolour?text1=${fison}&text2=${twoso}&apikey=apivinz`)
					if (fison > 10 || twoso > 10) return kill.reply(from, 'Desculpe, maximo de 10 letras.', id)
					 await kill.sendFileFromUrl(from, `${water.data.result}`, '', '', id) 
				} else {
					await kill.reply(from, `Para usar isso, adicione duas frases, separando elas pelo |.`, id)
				}
				break
		...
		case 'grafit':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.reply(from, mess.wait, id)
			if (args.length == 0) return kill.reply(from, 'Defina um nome para a logo!', id)
			const grafit = await axios.get(`https://api.zeks.xyz/api/grafiti?text=${body.slice(7)}&apikey=apivinz`)
			await kill.sendFileFromUrl(from, `${grafit.data.result}`, '', '', id) 
			break
...
//			case 'color':
				...
				if (args.length == 0) return kill.reply(from, 'Defina um nome para a logo!', id)
				kill.reply(from, mess.wait, id)
				const color = await axios.get(`https://api.zeks.xyz/api/colortext?text=${body.slice(7)}&apikey=apivinz`)
				await kill.sendFileFromUrl(from, `${color.data.result}`, '', 'Aqui está a logo.', id) 
				break
...
//				case 'bneon':
					...
					if (args.length == 0) return kill.reply(from, 'Defina um nome para a logo!', id)
					kill.reply(from, mess.wait, id)
					const bneon = await axios.get(`https://api.zeks.xyz/api/bneon?apikey=apivinz&text=${body.slice(7)}`)
					await kill.sendFileFromUrl(from, `${bneon.data.result}`, '', 'Aqui está a logo', id) 
					break
...
//				case 'light':
					...
					if (args.length == 0) return kill.reply(from, 'Defina um nome para a logo!', id)
					kill.reply(from, mess.wait, id)
					const light = await axios.get(`https://api.zeks.xyz/api/tlight?text=${body.slice(7)}&apikey=apivinz`)
					await kill.sendFileFromUrl(from, `${light.data.result}`, '', 'Aqui está a logo', id) 
					break
...
//				case 'ytsilver':
					...
					if (args.length == 0) return kill.reply(from, 'Defina um nome para a logo!', id)
					kill.reply(from, mess.wait, id)
					const ytsilver = await axios.get(`https://api.zeks.xyz/api/splaybutton?text=${body.slice(10)}&apikey=apivinz`)
					await kill.sendFileFromUrl(from, `${ytsilver.data.result}`, '', 'Aqui está a logo', id) 
					break
...
//				case 'ytgold':
					...
					if (args.length == 0) return kill.reply(from, 'Defina um nome para a logo!', id)
					kill.reply(from, mess.wait, id)
					const ytgold = await axios.get(`https://api.zeks.xyz/api/gplaybutton?text=${body.slice(8)}&apikey=apivinz`)
					await kill.sendFileFromUrl(from, `${ytgold.data.result}`, '', 'Aqui está a logo', id) 
					break
...
//				case '3d':
					...
					if (args.length == 0) return kill.reply(from, 'Defina um nome para a logo!', id)
					kill.reply(from, mess.wait, id)
					const tresd = await axios.get(`https://api.zeks.xyz/api/text3dbox?apikey=apivinz&text=${body.slice(4)}`)
					await kill.sendFileFromUrl(from, `${tresd.data.result}`, '', 'Aqui está a logo', id) 
					break
...
//				case 'harta':
					...
					if (args.length == 0) return kill.reply(from, 'Defina um nome para a logo!', id)
					kill.reply(from, mess.wait, id)
					const harta = await axios.get(`https://api.zeks.xyz/api/hartatahta?text=${body.slice(7)}&apikey=apivinz`)
					await kill.sendFileFromUrl(from, `${harta.data.result}`, '', 'Aqui está a logo', id) 
...
//				case 'matrix':
					...
					if (args.length == 0) return kill.reply(from, 'Defina um nome para a logo!', id)
					kill.reply(from, mess.wait, id)
					const gneon = await axios.get(`https://api.zeks.xyz/api/gneon?apikey=apivinz&text=${body.slice(8)}`)
					await kill.sendFileFromUrl(from, `${gneon.data.result}`, '', 'Aqui está a logo', id) 
					break
        ...
        case 'kick':
			...
			if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
				await kill.sendFile(from, './lib/media/img/banido.jpg',)
				await kill.sendPtt(from, './lib/media/tts/banido.mp3',)
				if (!isBotGroupAdmins) return kill.reply(from, mess.error.Ba, id)
				if (quotedMsg) {
		...
		case 'status':
			...
			const osName = require('os-name')
            const osname = osName()
			const deviceinfo = `┌────── •✧✧• ──────┐\n🥰 *Tempo Ativa* : ${formater(uptime)}\n🔋 *Nivel da Bateria* : ${device.battery}%\n🌐 *Tempo de resposta* : ${processTime(t, moment())} \n⏰ *Hora atual :* ${moment(t * 1000).format('HH:mm:ss')}\n🔘 *RAM:* ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB \/ ${Math.round(require('os').totalmem / 1024 / 1024)}MB\n💠 *CPU:* ${require('os').cpus()[0].model}\n💻 *Sistema Operacional:* ${osname} ${require('os').arch}\n└────── •✧✧• ──────┘`   
			const chatIds = await kill.getAllChatIds()
			const groups = await kill.getAllGroups()
			const blocked = await kill.getBlockedIds()
			const groupsIn = groups.filter(x => x.groupMetadata.participants.map(x => [botNumber, '5511911378544@c.us'].includes(x.id._serialized)).includes(true))
			kill.sendText(from, `⚫️ *STATUS DA AKAMI* ⚫️\n\n${deviceinfo}\n\n⚜️ Informação Chat's :\n📥 *${loadedMsg}* Mensagens carregadas\n📥 *${groupsIn.length}* Total de grupos que entrei\n📥 *${groups.length - groupsIn.length}* Total de grupos que sai\n📥 *${groups.length}* Total de grupos\n📥 *${chatIds.length}* Total de privados\n📥 *${chatIds.length - groupsIn.length}* Total de privados ativos\n⛔️ *${blocked.length}* Total de Bloqueados`)
			 }           
			break
        ...
        case 'tela':
            ...
            kill.sendFile(from, sesPic, 'session.png', 'Aqui está', id)
            break
		...
		case 'femboy':
			...
			if (isGroupMsg) {
				if (!isNsfw) return kill.reply(from, mess.error.Ac, id)
			const porn = await axios.get('https://meme-api.herokuapp.com/gimme/FemBoys')
			kill.sendFileFromUrl(from, porn.data.url, '', porn.data.title, id)
			} else {
				const porn = await axios.get('https://meme-api.herokuapp.com/gimme/FemBoys')
				kill.sendFileFromUrl(from, porn.data.url, '', porn.data.title, id)
		...
		case 'bblack':
			...
			if (isGroupMsg) {
				if (!isNsfw) return kill.reply(from, mess.error.Ac, id)
			const porn = await axios.get('https://meme-api.herokuapp.com/gimme/bigblackasses')
			kill.sendFileFromUrl(from, porn.data.url, '', porn.data.title, id)
			} else {
				const porn = await axios.get('https://meme-api.herokuapp.com/gimme/bigblackasses')
				kill.sendFileFromUrl(from, porn.data.url, '', porn.data.title, id)
		...
		case 'penis':
			...
			if (isGroupMsg) {
				if (!isNsfw) return kill.reply(from, mess.error.Ac, id)
			const porn = await axios.get('https://meme-api.herokuapp.com/gimme/penis')
			kill.sendFileFromUrl(from, porn.data.url, '', porn.data.title, id)
			} else {
				const porn = await axios.get('https://meme-api.herokuapp.com/gimme/penis')
				kill.sendFileFromUrl(from, porn.data.url, '', porn.data.title, id)
		...
		case 'wallmobile':
			...
			if (!isGroupMsg) {
				if (isNsfw) return kill.reply(from, mess.error.Ac, id)
			const wallm = await axios.get('https://meme-api.herokuapp.com/gimme/mobilewallpapers')
			kill.sendFileFromUrl(from, wallm.data.url, '', wallm.data.title, id)
			} else {
				const wallm = await axios.get('https://meme-api.herokuapp.com/gimme/iWallpaper')
				kill.sendFileFromUrl(from, wallm.data.url, '', wallm.data.title, id)
			}
			break
		...
		case 'nekowall' :
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.reply(from, mess.wait, id)
			const walnimemo = await axios.get('https://akaneko-api.herokuapp.com/api/mobileWallpapers')
			await kill.sendFileFromUrl(from, walnimemo.data.url, '', 'Sucesso, aqui está.', id)
			break
		...
		case 'randomsticker':
			...
			kill.sendStickerfromUrl(from, res.data.url,'shit.jpg', id);
			});
			break 
		...
		case 'nickff':
			...
			const hasifl = `*Nickname Aleatório de Freefire*\n\n\n*Resultado* : ${res.data.result}\n\nPor: Akami`;
			kill.sendText(from, hasifl, id)
			kill.sendPtt(from, './lib/media/tts/nobru.mp3', id)
			}) 
			break
		...
		case 'stickdarcula':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			if (args.length === 0) return kill.reply(from, 'Cadê o texto???', id)
			kill.reply(from, mess.wait, id)	
			const darculanyas = `https://carbonnowsh.herokuapp.com/?code=${body.slice(14)}&theme=darcula&backgroundColor=rgba(50, 50, 50, 150)`
			kill.sendFileFromUrl(from, darculanyas)
			break


		case 'apkpure':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			if (args.length === 0) return kill.reply(from, `Cadê o termo? , Exemplo : *${prefix}apkpure youtube*`, id)
			...
			try {
				...
				const dataplayx = dataplaix.data
				 let keluarplayx = `*「 PLAYSTORE 」*\n\n*Pesquisa sobre : ${keywotpx}*\n`
				for (let i = 0; i < dataplayx.result.length; i++) {
					keluarplayx += `\n─────────────────\n\n• *Nome* : ${dataplayx.result[i].title}\n• *URL* : ${dataplayx.result[i].url}\n• *Rating* : ${dataplayx.result[i].rating}\n\n\nBy : Akami\n`
				}
				await kill.sendFileFromUrl(from, dataplayx.result[0].thumbnail, `thumbnailapk.webp`, keluarplayx, id)
			} catch (err){
			console.log(err)
			}
			break
        ...
        case 'listgroup':
            if (!isOwner) return await kill.reply(from, ind.ownerOnly(), id)
                kill.getAllGroups().then((res) => {
                let gc = '*Lista de grupos*:\n'
                for (let i = 0; i < res.length; i++) {
                     gc += `\n\n*N°*: ${i+1}\n*Nome*: ${res[i].name}\n*Mensagens não lidas*: ${res[i].unreadCount} Mensagens\n\n──_──_──_──_──_──_──_──_──_──_──_──_──`
                }
                kill.reply(from, gc, id)
            })
        break
		...
		case 'xnxxapp':
            kill.reply(from, mess.wait, id)
          	await  kill.sendFileFromUrl(from, `https://piyobot.000webhostapp.com/XXNX%20Mod%20agustusan%20(SFILE.MOBI).apk`, 'xnxx.apk', id)
           	await  kill.reply(from, 'HUMMMM\'Vi que se interessou, ai está....', id)
           	.then(() => console.log('Enviei o arquivo APK com sucesso!'))
			break

		case 'reportbug':
		case 'report':
		case 'bug':
			if (args.length === 0) return kill.reply(from, 'Use .report / .reportbug / .bug [BUG]', id)
			kill.reply(from, mess.wait, id)
			const reporter = body.slice(8)
			await kill.sendText(ownerNumber, `Report feito por: : *${pushname}*\n\nBug : *${reporter}*\n\nNúmero do usuário : *wa.me/${sender.id.replace(/@c.us/g, '')}*`)
			kill.reply(from, 'Relatório enviado com sucesso ao Dono!\nObrigado por relatar..\npor favor aguarde uma resposta do proprietário!\n\n\n~Akami', id)
			break


		case 'card':
			const liputan = await axios.get('https://videfikri.com/api/ccgenerator').then(res => {
			const hasillip = `*[ Cartão Gerado ]*\n\n\n*Network* : _${res.data.result.card.network}_\n\n*Número* : _${res.data.result.card.number}_\n\n*CVV* : _${res.data.result.card.cvv}_\n\n*PIN* : _${res.data.result.card.pin}_\n\n*Balance* : _${res.data.result.card.balance}_`;
			kill.sendText(from, hasillip, id)
			}) 
			break

		case 'setstatus':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			if (!isOwner) return kill.reply(from, `Apenas meu dono pode usar!`, id)
			const setstat = body.slice(11)
			await kill.setMyStatus(setstat)
			kill.sendTextWithMentions(from, `Obrigado pelo o novo status @${sender.id.replace('@c.us','')} 😘`)
			break
		...
		case 'runtime':
			...
			const uptime = process.uptime()
			await kill.reply(from, `── *「 TEMPO ATIVA 」* ──\n\n ${formater(uptime)}`, id)
			break
		...
		case 'wa':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			await kill.reply(from, `*Este é o link do seu número do WhatsApp ${pushname}*\n\n*wa.me/${sender.id.replace(/[@c.us]/g, '')}*\n\n*ou*\n\n*api.whatsapp.com/send?phone=${sender.id.replace(/[@c.us]/g, '')}*\n\nFeito por Akmai`, id)
			break
        ...
        case 'regra':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			await kill.reply(from, `*⚠️ [ REGRAS ] ⚠️*\n\n*Olá ${pushname}, aqui está minhas regras principais.*\n\n*1. Não envie spam para a Akami.*\n _Sanções:_ *BLOCK TEMPORÁRIO*\n\n*2. Não telefone para a Akami ou para seu dono.*\n _Sanções:_ *BLOCK PERMANENTE*\n\n*3. Não explore a Akami.*\n _Sanções:_ *BLOCK TEMPORÁRIO*\n\nSe as regras forem compreendidas, pode usar a minha tabela!\nPrefixo: *${prefix}*`, id)
			break
        ...
        case 'help':
		case 'menu':
            const bots = `*Olá *${pushname}*!\n\n*Eu sou a Akami, uma BOT de WhatsApp.*\n\n Quer saber minha tabela de comandos?*\n\nDigite *${prefix}ajuda*`
            await kill.reply(from, bots , id)
            break
        ...
        case 'ajuda':
			...
			const allin = `💜 Olá "@${sender.id}" caralhoo!\n\n🧭 Levei ${processTime(t, moment())} segundos para te responder.\n\n⏰ Agora são exatas "${timed}".\n\n🥪 Abaixo estão minhas funções.\n\n\n`
            kill.sendTextWithMentions(from, allin + help, id)
            //kill.reply(from, `Obrigado(a) pelo o uso seu merda!`, id)
            break


		default:
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.sendText(from, cmdNotFound(command), id)
			break


		case 'stickers':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.sendText(from, stic, id)
			break
	
			
		case 'logos':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.sendText(from, maker, id)
			break
	
	
		case 'avançado':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.sendText(from, downa, id)
			break
	
	
		case 'animais':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.sendText(from, animal, id)
			break
	
	
		case 'estudos':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.sendText(from, estudo, id)
			break
	
	
		case 'painel':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.sendText(from, Akami, id)
			break
	
	
		case 'animes':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.sendText(from, animes, id)
			break
	
	
		case 'outros':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
			kill.sendText(from, outros, id)
			break
        ...
        case 'adulto':
			...
			kill.sendPtt(from, './lib/media/tts/adulto.mp3', id)
            break


        case 'letras':
			if (mute || pvmte) return console.log('Ignorando comando [Silence]')
            kill.sendText(from, letras, id)
            break
        ...
        case 'mutepv':
            if (isOwner) {
				if (args[0] == 'on') {
					...
					slce.push(pvmt)
					fs.writeFileSync('./lib/config/silence.json', JSON.stringify(slce))
					await kill.reply(from, 'Ele não poderá usar a Akami.', id)
				} else if (args[0] == 'off') {
					if (args.length == 0) return kill.reply(from, 'Você deve definir [on e off] e em seguida o número da pessoa sem - ou +.', id)
					const pvmt = body.slice(11) + '@c.us'
					...
					slce.splice(pvtnm, 1)
					fs.writeFileSync('./lib/config/silence.json', JSON.stringify(slce))
					await kill.reply(from, 'Ele poderá usar a Akami novamente.', id)
				} else {
